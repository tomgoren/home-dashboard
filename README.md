...
weather. It renders directly to DRM/KMS (or a legacy framebuffer): no
X11, Wayland, Chromium, Electron, Qt, or GTK.
...
- Real weather from [Open-Meteo](https://open-meteo.com/) (no API key),
  ...
  the network is down.
- Local time comes from the configured location's own UTC offset, not
  the device's system clock/timezone.
...
[mise](https://mise.jdx.dev) manages Python: `.mise.toml` pins the
interpreter and auto-creates/activates a `.venv`, so there's no manual
`venv`/`activate` step.

Get it running on the device itself, or provision a headless device
remotely over SSH from your own machine.
...
for you. If you already have a local `config.toml`, it copies over
automatically. Otherwise it prompts you to set the location on the
device directly.
...
driver, needing no display device. This bypasses `display/backend.py`
entirely: it verifies composition and data flow, not whether the
...
**A screen is attached and the app runs on it, but you can't look at it
right now.** `app/main.py` installs a `SIGUSR1` handler that dumps
